...
class Favicon implements \Magento\Framework\View\Page\FaviconInterface
{
    ...
    public function __construct(
        ...
        $fileStorageDatabase = null,
        \Magento\Framework\Filesystem $filesystem = null
    ) {
        $this->storeManager = $storeManager;
        $this->scopeConfig = $scopeConfig;
        $this->fileStorageDatabase = $fileStorageDatabase;
        $this->mediaDirectory = $filesystem ? $filesystem->getDirectoryRead(DirectoryList::MEDIA) : null;
    }
    ...
    {
        if ($this->fileStorageDatabase && $this->fileStorageDatabase->checkDbUsage() && $this->mediaDirectory && !$this->mediaDirectory->isFile($filename)) {
            $this->fileStorageDatabase->saveFileToFilesystem($filename);
        }
        return $this->mediaDirectory ? $this->mediaDirectory->isFile($filename) : false;
    }
}
