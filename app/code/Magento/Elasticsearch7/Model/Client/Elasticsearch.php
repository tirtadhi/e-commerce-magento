...
class Elasticsearch implements ClientInterface
{
    ...
    {
        return $this->normalizeArrayResponse($this->getElasticsearchClient()->suggest($query));
    }
    ...
    public function ping(): bool
    {
        if ($this->pingResult === null) {
            $this->pingResult = $this->normalizeBoolResponse(
                $this->getElasticsearchClient()->ping(['client' => ['timeout' => $this->clientOptions['timeout']]])
            );
        }
    ...
    {
        $stats = $this->normalizeArrayResponse(
            $this->getElasticsearchClient()->indices()->stats(['index' => $index, 'metric' => 'docs'])
        );
    ...
    {
        return $this->normalizeBoolResponse($this->getElasticsearchClient()->indices()->exists(['index' => $index]));
    }
    ...
    public function existsAlias(string $alias, string $index = ''): bool
    {
        ...
        return $this->normalizeBoolResponse($this->getElasticsearchClient()->indices()->existsAlias($params));
    }
    ...
    {
        return $this->normalizeArrayResponse($this->getElasticsearchClient()->indices()->getAlias(['name' => $alias]));
    }
    ...
    {
        return $this->normalizeArrayResponse($this->getElasticsearchClient()->search($query));
    }
    ...
    {
        return $this->normalizeArrayResponse($this->getElasticsearchClient()->indices()->getMapping($params));
    }

    /**
     * Normalize Elasticsearch client responses to bool across v7/v8 clients.
     *
     * @param mixed $response
     * @return bool
     */
    private function normalizeBoolResponse($response): bool
    {
        if (is_bool($response)) {
            return $response;
        }
    ...
    /**
     * Normalize Elasticsearch client responses to arrays across v7/v8 clients.
     *
     * @param mixed $response
     * @return array
     */
    private function normalizeArrayResponse($response): array
    {
